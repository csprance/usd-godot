...
if not exist "%USD_DEPS_DIR%\install\lib\tbb.lib" (
    ...
    cd build
    cmake .. -DTBB_TEST=OFF -DTBB_STRICT=OFF -DBUILD_SHARED_LIBS=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX="%USD_DEPS_DIR%\install" -DCMAKE_MSVC_RUNTIME_LIBRARY=MultiThreaded -DCMAKE_POLICY_DEFAULT_CMP0091=NEW -G "Visual Studio 17 2022" -A x64
